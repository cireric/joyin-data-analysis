...
阿里云百炼采用**按量计费**模式，无预付费资源包，根据实际 Token 消耗计费：

**计费规则**:
- 按输入 Token 和输出 Token 分别计费
- 部分模型支持阶梯计费（根据单次请求的输入 Token 数量）
- 支持 Batch 调用（50% 折扣）和上下文缓存（输入折扣）

**主流模型价格（中国内地）**:

| 模型 | 输入价格 | 输出价格 | 上下文 | 免费额度 |
|------|----------|----------|--------|----------|
| **qwen3.6-plus** | ¥2-8/百万 | ¥12-48/百万 | 1M | 100万 Token（90天） |
| **qwen3.5-plus** | ¥0.8-4/百万 | ¥4.8-24/百万 | 1M | 100万 Token（90天） |
| **qwen3-max** | ¥2.5-7/百万 | ¥10-28/百万 | 256K | 100万 Token（90天） |
| **qwen-turbo** | ¥0.3/百万 | ¥0.6-3/百万 | - | 100万 Token（90天） |
| **qwen3.6-flash** | ¥1.2-4.8/百万 | ¥7.2-28.8/百万 | 1M | 100万 Token（90天） |

**阶梯计费示例**（qwen3.5-plus）:
- 0-128K Token: 输入 ¥0.8/百万，输出 ¥4.8/百万
- 128K-256K Token: 输入 ¥2/百万，输出 ¥12/百万
- 256K-1M Token: 输入 ¥4/百万，输出 ¥24/百万

**Token Plan vs Coding Plan 对比**:

| 对比项 | Coding Plan | Token Plan（按量计费） |
|--------|-------------|------------------------|
| 计费方式 | 固定月费 | 按 Token 消耗计费 |
| 额度限制 | 请求次数限制 | 无限制，用多少付多少 |
| 适用场景 | 编程工具专用 | 通用 API 调用 |
| 模型选择 | 多模型可切换 | 百炼所有模型 |
| 团队共享 | 单人使用 | 可多人共享 |
| 性价比 | 高频使用更划算 | 低频使用更灵活 |
| 免费额度 | 无 | 100万 Token（90天） |

**费用估算**:
- 月消耗 1000万 Token（qwen3.5-plus，输入输出比 1:1）: 约 ¥28
- 月消耗 1亿 Token（qwen3.5-plus）: 约 ¥280
- 相比 Coding Plan Pro (¥200/月)，按量计费适合中低频使用
