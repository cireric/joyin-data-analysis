.PHONY: analyze clean crawl help
...
help:
	...
	@echo   make crawl url=URL [ARGS]                     # URL to Markdown
...
crawl:
	@.venv\Scripts\python.exe scripts/url2md.py "$(url)" $(if $(output),--output $(output)) $(if $(limit),--limit $(limit)) $(if $(delay),--delay $(delay)) $(if $(max_delay),--max-delay $(max_delay)) $(if $(filter $(1),--download-images),--download-images) $(if $(filter $(1),--resume),--resume) $(if $(state),--state $(state))
